  ...
  const handleReserveClick = async () => {
    ...
    try {
      ...
      const orderRes = await fetch("/api/razorpay/order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ amount: 19, currency: "INR", contact: phoneNumber, description: "Maths LTC Demo 2026" }),
      });
      ...
      const options: any = {
        ...
        handler: async function (response: any) {
          ...
          window.location.href = `/mathltc/payment/success.php?transactionId=${encodeURIComponent(
        ...
        modal: {
          ondismiss: function () {
            ...
            updatePaymentStatus("fail").finally(() => {
              window.location.href = `/mathltc/payment/failed.php?transactionId=${encodeURIComponent(
                `DISMISSED_${Date.now()}`
              )}`;
            });
          },
