...
export default function HeroSection({ onRegister, onGetCallback }: HeroSectionProps) {
  ...
  return (
    <section className="relative w-full bg-[#01317A] text-white overflow-hidden min-h-[450px] sm:min-h-[480px] md:min-h-[520px] lg:min-h-[450px] xl:min-h-[550px] py-6 sm:py-8 md:py-10 lg:py-12 xl:py-23">
      <div className="mx-auto flex w-full max-w-[1300px] lg:max-w-[1300px] xl:max-w-[1500px] 2xl:max-w-[1600px] flex-col gap-3 px-4 pt-6 pb-4 sm:gap-3 sm:px-5 sm:pt-7 sm:pb-4 md:flex-row md:items-center md:justify-center md:gap-0 md:px-6 md:pt-10 md:pb-6 lg:gap-0 lg:px-6 lg:pt-10 lg:pb-6 xl:gap-0 xl:px-8 xl:pt-14 xl:pb-0">
        {/* Left Column */}
        <div className={`w-full max-w-xl sm:max-w-lg md:max-w-xl lg:max-w-[580px] xl:max-w-[700px] mt-0 sm:-mt-3 md:-mt-4 lg:-mt-16 pl-4 sm:pl-6 md:pl-8 lg:pl-12 xl:pl-16 transition-all duration-[1000ms] ease-out ${entered ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-[80px]'}`}>
          {/* Main Headline */}
          ...
          {/* Sub-headline */}
          <p className={`mt-1 font-roboto font-black text-[22px] leading-[28px] tracking-[0.03em] text-[#FEFEFE] sm:text-[24px] sm:leading-[30px] md:text-[28px] md:leading-[34px] lg:text-[30px] lg:leading-[36px] xl:text-[40px] xl:leading-[45px] transition-all duration-[1000ms] ease-out ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'}`} style={{ transitionDelay: entered ? '200ms' : '0ms' }}>
            Live Classes
          </p>

          {/* Grade Level */}
          <p className={`mt-1 font-roboto font-medium italic text-[14px] leading-[20px] tracking-[0.03em] text-[#FEFEFE] sm:text-[16px] sm:leading-[22px] md:text-[18px] md:leading-[26px] lg:text-[17px] lg:leading-[28px] xl:text-[20px] xl:leading-[30px] transition-all duration-[1000ms] ease-out ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'}`} style={{ transitionDelay: entered ? '250ms' : '0ms' }}>
            Grades 8 - 10
          </p>

          {/* Bullet Points */}
          <ul className={`mt-4 space-y-1.5 font-roboto font-medium text-[13px] leading-[20px] tracking-[0.03em] text-[#FEFEFE] list-disc pl-5 sm:text-[14px] sm:leading-[22px] md:text-[16px] md:leading-[24px] lg:text-[15px] lg:leading-[26px] xl:text-[18px] xl:leading-[31px] transition-all duration-[1000ms] ease-out ${entered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'}`} style={{ transitionDelay: entered ? '300ms' : '0ms' }}>
            ...
            <li className={`transition-all duration-[800ms] ease-out ${entered ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-[20px]'}`} style={{ transitionDelay: entered ? '600ms' : '0ms' }}>Focus On Speed & Accuraccy</li>
            <li className={`transition-all duration-[800ms] ease-out ${entered ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-[20px]'}`} style={{ transitionDelay: entered ? '700ms' : '0ms' }}>Subjects - Maths, Physics & Chemistry</li>
          </ul>
        ...
        <div className={`relative w-full mt-4 sm:mt-5 md:mt-0 max-w-sm sm:max-w-md md:max-w-lg lg:max-w-[700px] xl:max-w-[900px] 2xl:max-w-[1000px] transition-all duration-[1200ms] ease-out ${entered ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 translate-x-[80px] scale-95'}`} style={{ transitionDelay: entered ? '200ms' : '0ms' }}>
          <div className="relative mx-auto flex w-full items-center justify-center">
            <Image
              ...
              height={1000}
              unoptimized
              className="relative w-full h-auto object-contain max-w-[380px] sm:max-w-[450px] md:max-w-[550px] lg:max-w-[650px] xl:max-w-[800px] 2xl:max-w-[950px]"
              priority
            />
          </div>
        </div>
