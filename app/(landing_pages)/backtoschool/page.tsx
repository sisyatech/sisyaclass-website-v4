...
import BacktoschoolContent from "@/components/backtoschool/BacktoschoolContent";
import Script from "next/script";
import { getPageSchemas } from "@/lib/schemaApi";
import SchemaInjector from "@/components/SchemaInjector";

export default async function BacktoschoolPage() {
  const schemas = await getPageSchemas('landing', 'backtoschool');
  
  return (
    <MobileMenuProvider>
      <SchemaInjector schemas={schemas} />
      <Script id="gtm-script-backtoschool" strategy="afterInteractive">
        {`(function(w, d, s, l, i) {
            w[l] = w[l] || [];
            w[l].push({
